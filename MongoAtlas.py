...
coins_list = ['bitcoin', 'ethereum', 'solana', 'ripple', 'cardano', 'dogecoin']
...
for coin in cont:
    ...
    csv_filename = os.path.join(folder_name, f"{coin_name.lower()}.csv")
    with open(csv_filename, 'w', encoding='utf-8') as csvfile:
        ...
        for k, v in flatten_data.items():
            v.update({'Coin': coin_name, 'Timestamp': datetime.utcfromtimestamp(k / 1000).strftime('%Y-%m-%d %H:%M:%S')})
        ...
        coin_collection = db[coin_name.lower()]
        ...
        for record in data:
            if coin_collection.count_documents(record, limit=1) == 0:
                ...
                print(f"Data for {coin_name} inserted into MongoDB Atlas successfully.")
            else:
                print(f"Data for {coin_name} already exists in MongoDB Atlas. Skipping insertion.")

        print(f"Coin data for '{coin_name}' added to MongoDB Atlas")
